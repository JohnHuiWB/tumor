...
from tumor import seg_data2 as data
...
class Unet(object):
    ...
    @staticmethod
    def _get_model():
        main_input = Input(data.PIXELS, name='main_input')
        auxiliary_input = Input(data.PIXELS, name='auxiliary_input')

        conv1_m = Conv2D(32, (3, 3), activation='relu', padding='same')(main_input)
        conv1_m = Conv2D(32, (3, 3), activation='relu', padding='same')(conv1_m)

        conv1_a = Conv2D(32, (3, 3), activation='relu', padding='same')(auxiliary_input)
        conv1_a = Conv2D(32, (3, 3), activation='relu', padding='same')(conv1_a)

        conv1 = concatenate([conv1_m, conv1_a], axis=3)
        ...
        model = Model(inputs=[main_input, auxiliary_input], outputs=[conv10])
    ...
    def train(self, batch_size=1, samples_per_epoch=1000, epochs=1):
        ...
        model.fit_generator(
            ...
            callbacks=[model_checkpoint,
                       history,
                       TensorBoard(log_dir=path.join(self._dir, './tensorboard'))]
        )
    ...
    def predict(self, x, x2):
        ...
        result = model.predict([x, x2], verbose=1)
        return result
...
if __name__ == '__main__':
    u = Unet()
    # u.draw_model()
    u.train(batch_size=4, samples_per_epoch=200, epochs=50)
    # u.continue_train('unet.h5', batch_size=4, samples_per_epoch=200, epochs=20)
    # print(u.eval())

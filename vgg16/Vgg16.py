...
def train(model, batch_size=4, samples_per_epoch=200, epochs=3):  #8,2000,5
    # history = History()
...
def continue_train(filename,batch_size=2, samples_per_epoch=200, epochs=1):
...
def test_10(filename):
    model =load_model(filename)
    for x,y in generate_arrays_from_file(FILENAME_T,10):
        r = model.predict(x)
        print(y,r)
        break
